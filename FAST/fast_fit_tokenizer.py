...
def fit_tokenizer(action_dataset, vocab_size=500):
    ...
    # Reshape data to match tokenization format: (num_sequences, sequence_length, action_dim)
    # FAST tokenizer expects 3D format during fitting, matching how we call it during inference
    if action_dataset.ndim == 2:
        print(f"📊 Reshaping data from {action_dataset.shape} to ({action_dataset.shape[0]}, {action_dataset.shape[1]}, 1)")
        action_dataset = action_dataset.reshape(action_dataset.shape[0], action_dataset.shape[1], 1)
...
def analyze_token_usage(tokenizer, sequences, q1, q99, expected_vocab_size=500):
    ...
    # Plot frequency of each token (0 to expected_vocab_size-1)
    if len(all_tokens) > 0:
        from collections import Counter
        token_freq = Counter(all_tokens)
        
        # Create frequency array for all possible tokens (0 to expected_vocab_size-1)
        frequencies = np.zeros(expected_vocab_size)
        for token_id, count in token_freq.items():
            if 0 <= token_id < expected_vocab_size:
                frequencies[token_id] = count
        
        # Plot bar chart of token frequencies
        plt.figure(figsize=(14, 6))
        plt.bar(range(expected_vocab_size), frequencies, color='steelblue', alpha=0.7, edgecolor='black', linewidth=0.5)
        plt.title(f'Frequency of each token (0-{expected_vocab_size-1}) across {len(sequences):,} sequences')
        plt.xlabel('Token ID')
        ...
        # Print statistics
        used_tokens = np.sum(frequencies > 0)
        print(f"📊 Token frequency statistics:")
        print(f"  - Tokens used: {used_tokens}/{expected_vocab_size}")
        print(f"  - Most frequent token: {np.argmax(frequencies)} (count: {int(np.max(frequencies)):,})")
        print(f"  - Least frequent (non-zero): {int(np.min(frequencies[frequencies > 0])) if used_tokens > 0 else 0}")
        print(f"  - Average frequency per used token: {np.mean(frequencies[frequencies > 0]):.2f}" if used_tokens > 0 else "  - No tokens used")
...
def main():
    ...
    training_data = generate_training_data(n_sequences=10000, sequence_length=1000)
    ...
    fitted_tokenizer = fit_tokenizer(normalized_data, vocab_size=500)


    # Step 4: Generate small test data for reconstruction demo
    print(f"\n📊 Step 4: Generating test data (3 sequences) for reconstruction...")
    test_sequences = generate_test_data(n_sequences=3, sequence_length=5000)
    ...
    print(f"\n📊 Step 8: Generating test data (10000 sequences) for token usage analysis...")
    usage_sequences = generate_test_data(n_sequences=1000, sequence_length=5000)
    analyze_token_usage(fitted_tokenizer, usage_sequences, q1, q99, expected_vocab_size=500)
...
#     # Step 3.5: Test detokenization on specific tokens and plot
#     print(f"\n🔍 Step 3.5: Testing detokenization on specific tokens...")
#     test_tokens = [256, 196, 174, 199, 163, 86, 196, 179, 194, 320, 195, 169, 194, 188, 471, 484, 450, 460, 434, 439, 424, 431, 421, 420, 486, 456, 426, 426, 442, 415, 405, 407, 407, 407, 407, 407, 407, 407, 407, 407, 403, 400, 398, 396, 423]
#     print(f"📊 Tokens to decode: {len(test_tokens)} tokens")
#     print(f"📊 Token range: [{min(test_tokens)}, {max(test_tokens)}]")
    
#     # Initialize tokenizer by encoding a dummy sequence first
#     # This sets time_horizon and action_dim internally
#     print(f"🔧 Initializing tokenizer with dummy sequence...")
#     dummy_seq = np.zeros((1, 5000, 1))  # Match training data shape: (batch, time_horizon, action_dim)
#     _ = fitted_tokenizer(dummy_seq)  # This initializes time_horizon=1000, action_dim=1
    
#     # Decode tokens (returns normalized data)
#     # The tokenizer now knows time_horizon=1000 and action_dim=1 from the dummy encode
#     # We can also pass these explicitly as fallback: time_horizon=1000, action_dim=1
#     try:
#         decoded_normalized = fitted_tokenizer.decode([test_tokens])
#     except AssertionError:
#         # Fallback: pass parameters explicitly
#         print(f"⚠️  Retrying decode with explicit parameters...")
#         decoded_normalized = fitted_tokenizer.decode([test_tokens])
#     if isinstance(decoded_normalized, np.ndarray) and decoded_normalized.ndim > 1:
#         decoded_normalized = decoded_normalized.flatten()
    
#     # Denormalize to original scale
#     decoded = quantile_denormalize(decoded_normalized, q1, q99, target_range=(-1, 1))
    
#     print(f"📊 Decoded sequence length: {len(decoded)}")
#     print(f"📊 Decoded range: [{decoded.min():.3f}, {decoded.max():.3f}]")
    
#     # Plot the detokenized sequence
#     plt.figure(figsize=(12, 6))
#     plt.plot(decoded, linewidth=2, color='steelblue', label='Detokenized Sequence')
#     plt.title(f'Detokenized Sequence from {len(test_tokens)} Tokens', fontsize=14, fontweight='bold')
...
#    # Step 3.5: Test detokenization on specific tokens and plot
#     print(f"\n🔍 Step 3.5: Testing detokenization on specific tokens...")
#     test_tokens = [194, 175, 435, 491, 194, 186, 443, 439, 417, 426, 418, 404, 399, 423]
...
#     # This sets time_horizon and action_dim internally
#     print(f"🔧 Initializing tokenizer with dummy sequence...")
#     dummy_seq = np.zeros((1, 50, 1))  # Match training data shape: (batch, time_horizon, action_dim)
#     _ = fitted_tokenizer(dummy_seq)  # This initializes time_horizon=1000, action_dim=1
...
#         # Fallback: pass parameters explicitly
#         print(f"⚠️  Retrying decode with explicit parameters...")
#         decoded_normalized = fitted_tokenizer.decode([test_tokens], time_horizon=50, action_dim=1)
#     if isinstance(decoded_normalized, np.ndarray) and decoded_normalized.ndim > 1:
#         decoded_normalized = decoded_normalized.flatten()
