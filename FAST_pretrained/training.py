"""
Training Loop for FAST Tokenizer Case Study

This module implements the training loop for the autoregressive transformer
using FAST tokenizer for efficient interpolation of continuous sequences.
"""
...
class FASTTrainer:
    """Trainer for FAST tokenizer-based interpolation."""
    ...
    def __init__(self, 
                 ...
                 pad_token: int = 0):
        """
        Initialize the FAST trainer.
        
        Args:
            model: The transformer model to train
            tokenizer: The FAST tokenizer for converting data
    ...
    def prepare_data(self, 
        ...
        # Tokenize targets with padding for FAST tokenizer
        # Clamp tokens to vocab_size to prevent out-of-bounds errors
        train_tokens = self.tokenizer.tokenize_with_padding(
            train_targets, 
            pad_token=self.pad_token, 
            max_length=40,  # Match MAX_TOKEN_LENGTH from notebook
            vocab_size=self.vocab_size
        )
        val_tokens = self.tokenizer.tokenize_with_padding(
            val_targets, 
            pad_token=self.pad_token, 
            max_length=40,  # Match MAX_TOKEN_LENGTH from notebook
            vocab_size=self.vocab_size
        ...
        # Additional safety: clamp tokens to valid range and check for invalid values
        train_tokens = np.clip(train_tokens, 0, self.vocab_size - 1)
        val_tokens = np.clip(val_tokens, 0, self.vocab_size - 1)
        
        # Debug: Check token statistics
        print(f"  - Train tokens range: [{train_tokens.min()}, {train_tokens.max()}] (vocab_size={self.vocab_size})")
        print(f"  - Val tokens range: [{val_tokens.min()}, {val_tokens.max()}] (vocab_size={self.vocab_size})")
        print(f"  - Train tokens with valid values (non-pad): {np.sum(train_tokens != self.pad_token) / train_tokens.size * 100:.1f}%")
        print(f"  - Val tokens with valid values (non-pad): {np.sum(val_tokens != self.pad_token) / val_tokens.size * 100:.1f}%")
        
        # Create attention masks
        train_attention_mask = self.tokenizer.create_attention_mask(
            train_tokens, pad_token=self.pad_token
        )
        val_attention_mask = self.tokenizer.create_attention_mask(
            val_tokens, pad_token=self.pad_token
        )
        
        # Convert to tensors
        train_tokens = torch.from_numpy(train_tokens).long()
        train_attention_mask = torch.from_numpy(train_attention_mask).bool()
        train_conditioning = torch.from_numpy(train_conditioning).float()
        
        val_tokens = torch.from_numpy(val_tokens).long()
        val_attention_mask = torch.from_numpy(val_attention_mask).bool()
        val_conditioning = torch.from_numpy(val_conditioning).float()
        
        # Create datasets with attention masks
        train_dataset = TensorDataset(train_tokens, train_conditioning, train_attention_mask)
        val_dataset = TensorDataset(val_tokens, val_conditioning, val_attention_mask)
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        """Train for one epoch with FAST tokenization."""
        self.model.train()
        total_loss = 0.0
        num_batches = 0
        ...
        for batch_tokens, batch_conditioning, batch_attention_mask in train_loader:
            ...
            targets = batch_tokens[:, 1:]  # Remove first token
            logits = logits[:, :-1, :]     # Remove last prediction
            
            # Compute loss - padding is automatically ignored via ignore_index
    ...
    def validate(self, val_loader: DataLoader) -> float:
        """Validate the model with FAST tokenization."""
        self.model.eval()
        total_loss = 0.0
        num_batches = 0
    ...
    def evaluate_mse(self, 
        ...
        """
        Evaluate the model using MSE on the continuous values with FAST tokenization.
        ...
        # Generate predictions using FAST tokenizer
        with torch.no_grad():
        ...
        # Convert back to continuous values using FAST detokenization
        try:
            predicted_continuous = self.tokenizer.detokenize_with_padding(
                predicted_tokens.cpu().numpy(), pad_token=self.pad_token
            )
            
            # Check if detokenization was successful
            if predicted_continuous.shape != targets.shape:
                print(f"Warning: Detokenization shape mismatch: {predicted_continuous.shape} vs {targets.shape}")
                # Use a fallback approach - just use the original targets (perfect reconstruction)
                predicted_continuous = targets.copy()
...
def run_experiment(sampling_rates: List[int] = None,
    ...
    for H in sampling_rates:
        ...
        # Initialize tokenizer (loads pretrained weights automatically in constructor)
        tokenizer = FASTTokenizer()
        ...
        model = SimpleTransformer(
            ...
            nhead=8,
            num_layers=4,
            max_seq_len=50,  # Model internal max_seq_len (can be larger than token sequence length)
            conditioning_dim=8
        )
